...
def generate_samples(ori_data):
  ...
  def discriminator (X, T):
    ...
    with tf.variable_scope("discriminator", reuse = tf.AUTO_REUSE):
        # RNN network
        d_cell = tf.nn.rnn_cell.MultiRNNCell([rnn_cell(module_name, hidden_dim) for _ in range(3)])
        d_outputs, d_last_states = tf.nn.static_rnn(d_cell, [X[:, i, :] for i in range(max_seq_len)], dtype=tf.float32, sequence_length = T)
